...
const envPath = process.env.GTE_AGENT_BIN_PATH
const scriptDir = path.dirname(fs.realpathSync(__filename))
const cached = path.join(scriptDir, ".gte-agent")
const platform = { darwin: "darwin", linux: "linux", win32: "windows" }[os.platform()] || os.platform()
const arch = { x64: "x64", arm64: "arm64", arm: "arm" }[os.arch()] || os.arch()
const base = "@gte-agent/cli-" + platform + "-" + arch
const binary = platform === "windows" ? "gte-agent.exe" : "gte-agent"
...
if (!resolved) {
  console.error(
    "It seems that your package manager failed to install the right gte-agent CLI package. Try manually installing " +
