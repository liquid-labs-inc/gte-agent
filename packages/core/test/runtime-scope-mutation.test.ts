...
function provide(directory: string) {
  return Effect.provide(
    RuntimeScopeMutation.layer.pipe(
      Layer.provide(
        Layer.mergeAll(
          FSUtil.defaultLayer,
...
describe("RuntimeScopeMutation", () => {
  it.live("resolves an active relative existing file target", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        ...
        const plan = yield* (yield* RuntimeScopeMutation.Service).resolve({ path: "hello.txt" })
        ...
        expect(yield* (yield* RuntimeScopeMutation.Service).revalidate(plan)).toMatchObject({
  ...
  it.live("resolves an active relative prospective file target", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        ...
        const plan = yield* (yield* RuntimeScopeMutation.Service).resolve({ path: path.join("src", "new.txt") })
  ...
  it.live("rejects a relative lexical escape instead of promoting it to external authority", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        const error = yield* Effect.flip((yield* RuntimeScopeMutation.Service).resolve({ path: "../outside.txt" }))
        expect(error).toMatchObject({ _tag: "RuntimeScopeMutation.PathError", reason: "relative_escape" })
      }).pipe(provide(directory)),
    ),
  )
    ...
    withTmp((directory) => {
      ...
      return Effect.gen(function* () {
        ...
        const error = yield* Effect.flip(
          (yield* RuntimeScopeMutation.Service).resolve({ path: path.join("escape", "new.txt") }),
        )
        expect(error).toMatchObject({ _tag: "RuntimeScopeMutation.PathError", reason: "runtime_scope_escape" })
        yield* Effect.promise(() => fs.rm(outside, { recursive: true, force: true }))
      }).pipe(provide(directory))
    }),
  )

  it.live("accepts an explicit absolute in-runtimeScope target without external approval", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        const targetPath = path.join(directory, "new.txt")
        const plan = yield* (yield* RuntimeScopeMutation.Service).resolve({ path: targetPath })
        expect(plan.target).toMatchObject({
    ...
    withTmp((directory) =>
      withTmp((outside) =>
        Effect.gen(function* () {
          ...
          const service = yield* RuntimeScopeMutation.Service
          ...
          expect(error).toMatchObject({ _tag: "RuntimeScopeMutation.RevalidationError" })
        }).pipe(provide(directory)),
      ),
    ),
  )
    ...
    withTmp((directory) =>
      Effect.gen(function* () {
        ...
        expect(error).toMatchObject({
          _tag: "RuntimeScopeMutation.RevalidationError",
  ...
  test("keeps project references outside the mutation input API", () => {
    expect(Object.keys(RuntimeScopeMutation.ResolveInput.fields)).toEqual(["path", "kind"])
    expect(Schema.decodeUnknownSync(RuntimeScopeMutation.ResolveInput)({ path: "README.md", reference: "docs" })).toEqual({
      path: "README.md",
    })
  })
})
