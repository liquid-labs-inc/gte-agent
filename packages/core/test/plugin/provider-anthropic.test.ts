...
import { AnthropicPlugin } from "@gte-agent/core/plugin/provider/anthropic"
...
describe("AnthropicPlugin", () => {
  it.effect("applies legacy beta headers", () =>
    Effect.gen(function* () {
      ...
      expect((yield* catalog.provider.get(Provider.ID.anthropic)).request.headers["anthropic-beta"]).toBe(
        "interleaved-thinking-2025-05-14,fine-grained-tool-streaming-2025-05-14",
      )
      expect((yield* catalog.provider.get(Provider.ID.anthropic)).request.headers.Existing).toBe("1")
    }),
  )
  ...
  it.effect("ignores non-Anthropic providers", () =>
    Effect.gen(function* () {
      ...
      expect((yield* catalog.provider.get(Provider.ID.openai)).request.headers["anthropic-beta"]).toBeUndefined()
    }),
  )
  ...
  it.effect("creates Anthropic SDKs with the model provider ID as the SDK name", () =>
    Effect.gen(function* () {
      ...
      yield* plugin.add(AnthropicPlugin)
      yield* plugin.add({
        id: Plugin.ID.make("anthropic-sdk-inspector"),
