...
import { GithubCopilotPlugin } from "@gte-agent/core/plugin/provider/github-copilot"
...
describe("GithubCopilotPlugin", () => {
  ...
  it.effect("selects languageModel with the API model ID when responses and chat are absent", () =>
    Effect.gen(function* () {
      ...
      yield* plugin.trigger(
        "aisdk.language",
        {
          model: model("github-copilot", "alias", { api: { id: Model.ID.make("claude-sonnet-4") } }),
          sdk: { languageModel: fakeSelectorSdk(calls).languageModel },
          options: {},
        },
        {},
  ...
  it.effect("uses the API model ID when selecting responses or chat", () =>
    Effect.gen(function* () {
      ...
      yield* plugin.trigger(
        "aisdk.language",
        {
          model: model("github-copilot", "default", { api: { id: Model.ID.make("gpt-5") } }),
      ...
      yield* plugin.trigger(
        "aisdk.language",
        {
          model: model("github-copilot", "small", { api: { id: Model.ID.make("gpt-5-mini") } }),
          sdk: fakeSelectorSdk(calls),
          options: {},
        },
        {},
      ...
      yield* plugin.trigger(
        "aisdk.language",
        {
          model: model("github-copilot", "sonnet", { api: { id: Model.ID.make("claude-sonnet-4") } }),
          sdk: fakeSelectorSdk(calls),
          options: {},
        },
        {},
  ...
  it.effect("disables gpt-5-chat-latest before Copilot language selection", () =>
    Effect.gen(function* () {
      ...
      expect(
        (yield* catalog.model.get(Provider.ID.make("github-copilot"), Model.ID.make("gpt-5-chat-latest"))).enabled,
      ).toBe(false)
    }),
  )
  ...
  it.effect("does not disable gpt-5-chat-latest for non-Copilot providers", () =>
    Effect.gen(function* () {
      ...
      expect(
        (yield* catalog.model.get(Provider.ID.make("custom-copilot"), Model.ID.make("gpt-5-chat-latest"))).enabled,
      ).toBe(true)
    }),
  )
