...
import { ConfigProvider } from "@gte-agent/core/config/provider"
import { ConfigMigrateV1 } from "@gte-agent/core/v1/config/migrate"
import { ConfigV1 } from "@gte-agent/core/v1/config/config"
...
function testLayer(
  ...
  return Config.runtimeScopeLayer.pipe(
...
describe("Config", () => {
  ...
  it.effect("migrates arbitrary v1 configuration into valid configuration", () =>
    Effect.sync(() => {
      FastCheck.assert(
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(
                path.join(tmp.path, "gte-agent.json"),
                JSON.stringify({ $schema: "middle", providers: { middle: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "gte-agent.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "gte-agent.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
  ...
  it.live("accepts $schema metadata without writing it into config files", () =>
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          const file = path.join(tmp.path, "gte-agent.json")
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(path.join(tmp.path, "gte-agent.json"), "{ invalid"),
              fs.writeFile(path.join(tmp.path, "gte-agent.jsonc"), JSON.stringify({ providers: { invalid: true } })),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "gte-agent.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .gte-agent configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".gte-agent"), { recursive: true })
            await fs.mkdir(path.join(directory, ".gte-agent"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "gte-agent.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "gte-agent.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "gte-agent.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "gte-agent.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "config.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".gte-agent", "gte-agent.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".gte-agent", "gte-agent.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".gte-agent")),
              AbsolutePath.make(path.join(directory, ".gte-agent")),
            ])
