...
describe("RuntimeScopeSearch", () => {
  it.live("searches files in the active RuntimeScope with structured bounded results", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        ...
        const result = yield* (yield* RuntimeScopeSearch.Service).files({ pattern: "*.ts" })
  ...
  it.live("greps the RuntimeScope, exact relative files and directories, and include globs", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        yield* Effect.promise(async () => {
          await fs.mkdir(path.join(directory, "src"))
  ...
  it.live("does not discover hidden files during broad searches", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        yield* Effect.promise(async () => {
      ...
      Effect.gen(function* () {
        yield* Effect.promise(async () => {
          ...
          await fs.writeFile(
            ...
            `needle ${"x".repeat(RuntimeScopeSearch.MAX_LINE_PREVIEW_LENGTH)}\n`,
          )
        })
        const search = yield* RuntimeScopeSearch.Service
        const files = yield* search.files({ pattern: "*.txt", limit: 2 })
        const hardCappedFiles = yield* search.files({ pattern: "*.txt", limit: RuntimeScopeSearch.MAX_RESULT_LIMIT + 1 })
        const hardCappedGrep = yield* search.grep({ pattern: "needle", limit: RuntimeScopeSearch.MAX_RESULT_LIMIT + 1 })
        ...
        expect(hardCappedFiles.items).toHaveLength(RuntimeScopeSearch.MAX_RESULT_LIMIT)
        expect(hardCappedFiles.truncated).toBe(true)
        expect(hardCappedGrep.items).toHaveLength(RuntimeScopeSearch.MAX_RESULT_LIMIT)
        expect(hardCappedGrep.truncated).toBe(true)
        expect(grep.items[0].lines).toHaveLength(RuntimeScopeSearch.MAX_LINE_PREVIEW_LENGTH)
  ...
  it.live("reports invalid regex as a typed failure", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        ...
        const exit = yield* (yield* RuntimeScopeSearch.Service).grep({ pattern: "[" }).pipe(Effect.exit)
  ...
  it.live("rejects oversized ripgrep JSON records before durable projection", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        ...
        const exit = yield* (yield* RuntimeScopeSearch.Service).grep({ pattern: "needle" }).pipe(Effect.exit)
  ...
  it.live("rejects an approved root swapped to a symlink before ripgrep traversal", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        ...
        expect(
          Exit.isFailure(yield* (yield* RuntimeScopeSearch.Service).files({ pattern: "*" }, approved).pipe(Effect.exit)),
        ).toBe(true)
        yield* Effect.promise(() => fs.rm(outside, { recursive: true, force: true }))
      }).pipe(provide(directory)),
    ),
  ...
  it.live("honors a pre-aborted cancellation signal", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        ...
        controller.abort()
        const exit = yield* (yield* RuntimeScopeSearch.Service)
  ...
  test("exposes schema-testable search bounds", () => {
    const decode = Schema.decodeUnknownSync(RuntimeScopeSearch.FilesInput)
    expect(RuntimeScopeSearch.DEFAULT_RESULT_LIMIT).toBe(100)
    expect(RuntimeScopeSearch.MAX_RESULT_LIMIT).toBe(100)
    expect(RuntimeScopeSearch.MAX_LINE_PREVIEW_LENGTH).toBe(2_000)
    expect(() => decode({ pattern: "*", limit: RuntimeScopeSearch.MAX_RESULT_LIMIT + 1 })).toThrow()
  })
})
