...
import { CrossSpawnSpawner } from "@gte-agent/core/cross-spawn-spawner"
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["GTE_AGENT_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { GTE_AGENT_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("GTE_AGENT_TEST_SHELL=ok")
      }),
    )
