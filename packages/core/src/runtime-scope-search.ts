export * as RuntimeScopeSearch from "./runtime-scope-search"
...
/**
 * RuntimeScope-scoped raw search substrate. Search authority is selected only by
 * FileSystem, preserving RuntimeScope-relative paths and named read
...
export class File extends Schema.Class<File>("RuntimeScopeSearch.File")({
  path: RelativePath,
...
export class Submatch extends Schema.Class<Submatch>("RuntimeScopeSearch.Submatch")({
  text: Schema.String,
  start: NonNegativeInt,
  end: NonNegativeInt,
}) {}

export class Match extends Schema.Class<Match>("RuntimeScopeSearch.Match")({
  path: RelativePath,
  canonical: Schema.String,
  resource: Schema.String,
  lines: Schema.String,
...
export class FilesResult extends Schema.Class<FilesResult>("RuntimeScopeSearch.FilesResult")({
  items: Schema.Array(File),
  truncated: Schema.Boolean,
  partial: Schema.Boolean,
}) {}

export class GrepResult extends Schema.Class<GrepResult>("RuntimeScopeSearch.GrepResult")({
...
export class Service extends Context.Service<Service, Interface>()("@gte-agent/RuntimeScopeSearch") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const candidate = Effect.fnUntraced(function* (root: FileSystem.RootTarget, cwd: string, value: string) {
      ...
      const canonical = yield* Effect.promise(() => nodeFs.realpath(absolute)).pipe(Effect.catch(() => Effect.void))
      if (!canonical || !FSUtil.contains(root.root, canonical)) return
      const info = yield* Effect.promise(() => nodeFs.stat(canonical)).pipe(Effect.catch(() => Effect.void))
    ...
    return Service.of({
      files: Effect.fn("RuntimeScopeSearch.files")(function* (input, approvedRoot) {
      ...
      grep: Effect.fn("RuntimeScopeSearch.grep")(function* (input, approvedRoot) {
        const root = yield* filesystem.revalidateRoot(approvedRoot ?? (yield* filesystem.resolveRoot(input)))
