export * as RuntimeScopeMutation from "./runtime-scope-mutation"
 ...
 * inside the active RuntimeScope. Absolute paths outside it require separate
...
export class PathError extends Schema.TaggedErrorClass<PathError>()("RuntimeScopeMutation.PathError", {
  path: Schema.String,
...
export class RevalidationError extends Schema.TaggedErrorClass<RevalidationError>()(
  "RuntimeScopeMutation.RevalidationError",
  {
    path: Schema.String,
...
export interface Target {
  ...
  readonly type?:
    ...
    | "Socket"
    | "Unknown"
  /** Permission resource: RuntimeScope-relative for internal paths, canonical for external paths. */
  readonly resource: string
...
export interface Interface {
   ...
   * paths must stay inside the RuntimeScope. Absolute paths outside it require
...
export class Service extends Context.Service<Service, Interface>()("@gte-agent/RuntimeScopeMutation") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const locationRoot = yield* fs.realPath(runtimeScope.directory)
    ...
    const assertLocationIdentity = Effect.fnUntraced(function* (requested: string) {
      ...
      return yield* new PathError({ path: requested, reason: "runtime_scope_identity_changed" })
    })
    ...
    const resolve = Effect.fn("RuntimeScopeMutation.resolve")(function* (input: ResolveInput) {
      yield* assertLocationIdentity(input.path)
      const relative = !path.isAbsolute(input.path)
      const absolute = path.resolve(runtimeScope.directory, input.path)
      const lexicallyInternal = FSUtil.contains(runtimeScope.directory, absolute)
      ...
      if (lexicallyInternal && !FSUtil.contains(locationRoot, resolved.canonical)) {
        return yield* new PathError({ path: input.path, reason: "runtime_scope_escape" })
      }
     ...
     */
    const revalidate = Effect.fn("RuntimeScopeMutation.revalidate")(function* (plan: Plan) {
