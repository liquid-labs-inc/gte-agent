...
```txt
Public Tool.make      NativeTool value      ApplicationTools      RuntimeScope built-ins       RuntimeScope ToolRegistry                        Session runner
        │                     │                     │                      │                         │                                         │
        ├─ construct ─────────▶                     │                      │                         │                                         │
        │                     │                     │                      │                         │                                         │
        │                     ├─ scoped attach ─────▶                      │                         │                                         │
...
- `native.ts` defines the plain Core-native executable value exposed publicly as `Tool.make(...)`. It combines an `@gte-agent-ai/llm` model-facing definition with a Session-aware handler.
...
- `registry.ts` is the single execution registry. Each RuntimeScope owns one registry, its built-in contributions, effective precedence, input/output validation, permissions, and settlement.

`ToolRegistry.Entry` is intentionally more powerful than the public native tool value. Internal RuntimeScope tools may use Core-owned capabilities such as `assertPermission`; embedding applications receive only the narrow public execution context.

## Placement And Layers

- `ApplicationTools.Service` is process-scoped and must be shared by current and future Locations.
- `ToolRegistry.Service` is RuntimeScope-scoped because built-in handlers close over RuntimeScope services such as filesystem, permissions, and tool-output storage.
- `RuntimeScopeServiceMap` constructs fresh RuntimeScope services while receiving the shared `ApplicationTools.Service` as a dependency.
- `GTE Agent.layer` exposes the same shared application-tool service through `gte-agent.tools.attach(...)`.
- `ToolRegistry.defaultLayer` creates isolated application-tool state. It is suitable for self-contained consumers and tests, but not when attachments must be shared with a separately constructed `RuntimeScopeServiceMap`.

Do not make `ToolRegistry` process-global. Do not move RuntimeScope resources into `ApplicationTools`. Do not construct independent `ApplicationTools.layer` instances when the caller expects one attachment to appear across Locations.

## Contribution And Precedence

Built-in RuntimeScope tools contribute through `ToolRegistry.contribute(...)`. Application tools attach through `ApplicationTools.attach(...)`, exposed publicly as `gte-agent.tools.attach(...)`.
...
- A RuntimeScope tool always takes precedence over an application tool with the same name.
...
```txt
Embedding App               ApplicationTools       RuntimeScope ToolRegistry                 Session Runner
      │                             │                        │                                  │
      ├─ attach({ opencord_run }) ──▶                        │                                  │
      │                             │                        │                                  │
      │                             │                        ◀─ definitions() ──────────────────┤
...
tool/
  ...
  registry.ts           RuntimeScope-scoped effective lookup, validation, and execution
  builtins.ts           shipped RuntimeScope tool layer composition
  read.ts, bash.ts, ... individual RuntimeScope-scoped built-in contributions
```

Keep model/provider-neutral tool schemas and output projection in `@gte-agent-ai/llm`. Keep Session identity, permissions, RuntimeScope precedence, and settlement in Core.
...
```txt
            ...
            ▼                                         ▼                                        ▼
╭───────────────────────╮                ╭────────────────────────╮                ╭───────────────────────╮
│ Process contributions │                │ RuntimeScope contributions │                │ Session contributions │
╰───────────┬───────────╯                ╰────────────┬───────────╯                ╰───────────┬───────────╯
            │                                         │                                        │
            │                                         │
            ╰─────────────────────────────────────────◀─ ─ ─ ─ ─ ─ ─ ─  future ─ ─ ─ ─ ─ ─ ─ ─ ╯
...
- **Hierarchical overlays:** If a third contribution scope becomes real, consider one registry abstraction with process, RuntimeScope, and Session overlays rather than adding another special registry service.
- **Plugin tools:** Reuse the existing native tool value for restricted handlers and `ToolRegistry.Entry` for trusted Core-owned capabilities. Choose process or RuntimeScope contribution lifetime explicitly.
