...
Phase 1 was provisionally accepted on 2026-06-11 (Milestone 6 record: `docs/m_6-acceptance-record.md`), then reopened with Milestone 7: every agent reply was still the hardcoded demo stream, and Phase 1 could not close while the agent had no real LLM. Milestone 7 is now implemented and its acceptance record landed (`docs/m_7-acceptance-record.md`), closing Phase 1 again pending the manual interactive checks listed in that record. Phase 1's end condition is that the stripped OpenCode fork is a runnable, read-only GTE Agent terminal product with a real model behind it:
...
Phase 1 is composed of Milestones 1 through 7, all complete: 1, 2, and 3 narrowed the repo, stabilized the canonical runtime, and added the auth/authority stub contract; 4, 5, and 6 added the `gta` TUI, read-only GTE data tools and panels, and whole-phase acceptance of those surfaces; 7 added real LLM responses, `/models`, provider auth, and prompt autocomplete (plan: `docs/m_7-llm-models-auth-plan.md`; acceptance: `docs/m_7-acceptance-record.md`, whose manual interactive checks remain outstanding). There is no production user base and no migration compatibility burden for old OpenCode, V1, workspace, share, or account SQLite chains before MVP.
...
- Milestone 9: Self-Authored Tools (Tool Workshop). See `docs/m_9-self-authored-tools-plan.md` and `docs/m_9-acceptance-record.md`.
...
Milestone 9 gave the agent self-authored tools and closed a production gap: `websearch` and `webfetch` (implemented and tested since the fork, but composed only into the built-ins layer the server never builds) now reach the model through the production registry, and a `tool_workshop` tool lets the model define new tools at runtime — a name, a description, a flat parameter schema, and a JavaScript body persisted as JSON under `~/.gte-agent/tools/` (project `.gte-agent/tools/` is also discovered and wins collisions). Tool code executes in a per-invocation Bun worker behind the same hardening as workflow scripts (the sanitize pass and static guard were extracted to a shared `sandbox/` module), with exactly two bindings: `params` and `gte(name, args)`, a host proxy into the session's own tool registry allowlisted to the read-only `gte_*` data tools and capped per invocation. Created tools register live (callable the next turn, no restart), survive restarts through disk discovery, cannot claim shipped tool names, cannot be created by workflow agents, and disappear behind `GTE_AGENT_DISABLE_DYNAMIC_TOOLS` / `dynamicTools.enabled: false`. Its acceptance record is `docs/m_9-acceptance-record.md` (2026-06-12). The read-only data boundary is unchanged: a dynamic tool can read public data and compute, nothing else.
