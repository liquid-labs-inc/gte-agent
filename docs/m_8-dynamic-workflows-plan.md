...
- The curated catalog defines reasoning-effort variants per Anthropic model: `low`/`medium`/`high`/`xhigh`/`max` for adaptive-efforts models (Fable 5, Opus 4.8), `high`/`max` (budgeted legacy thinking) for Sonnet 4.6 and Haiku 4.5. `Session.model.variant` flows from model selection through `SessionRunner` into the LLM request; an unset variant means no thinking payload, exactly as today.
